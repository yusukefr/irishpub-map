...
import { getContentPublicationMissingFields, hasOnlySafeMarkdownUrls } from "../../apps/web/app/lib/content/validation";
...
describe("editorial content validation", () => {
  ...
  it("allows HTTP(S), root-relative, and fragment Markdown URLs", () => {
    expect(
      hasOnlySafeMarkdownUrls(
        "[site](https://example.com) [local](/discover) [section](#history)\n[ref]: http://example.com\n<https://example.com>",
      ),
    ).toBe(true);
  });

  it("rejects dangerous and unsupported Markdown URL schemes, including encoded schemes", () => {
