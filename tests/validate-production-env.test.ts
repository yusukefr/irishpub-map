...
import { isProductionApiKeyConfigured, isProductionEnvironment, validateProductionEnvironment } from "../scripts/validate-production-env.mjs";
...
describe("validate-production-env", () => {
  ...
  it("requires a non-empty API key only in Production", () => {
    ...
    expect(validateProductionEnvironment({ VERCEL_ENV: "production", IRISHPUB_MAP_API_KEY: "test-only-api-key" })).toBe(true);
