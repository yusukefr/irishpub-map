...
マイグレーション010は既存コンテンツを移行せず、Editorial Contentの2テーブルだけを追加します。適用後は、テーブル・制約・許可Locale・外部キーと適用履歴をverify SQLで確認します。Content RepositoryとMarkdown RendererはIssue #343、Admin APIはIssue #344、Admin UIはIssue #345で実装し、既存MDX GuideはIssue #380でNeonへ移行、Issue #381で公開取得元をNeonへ切り替えました。Admin UIはRepository / Service / APIを介して日英ContentをDraft保存・公開し、認証済み画面内のPreviewでは公開Repositoryや公開Cacheを経由しません。
