...
- `/admin/pubs` で店舗一覧を確認し、Neon が設定されている場合は店舗を追加・編集・削除する
- 管理APIでは公開・非公開の両方を取得し、新規店舗は非公開で作成する
- 管理者認証または `DATABASE_URL` が未設定の場合、書き込みを拒否して閲覧・設定案内に限定する
- 管理マスタAPIから都道府県、市区町村、タグ、営業ステータスを参照する。タグ・ステータスのCRUDは後続対応とする

### 管理画面改修で採用する運用

Issue #273で公開状態のDB保持、公開APIの絞り込み、管理取得への状態追加、新規店舗を非公開にする既定値を実装しました。公開切替UIとAPI、および日本語店舗名だけで保存するためのNULL制約変更はまだ実装されていません。親Issue #264の後続実装では、所在地・座標・営業ステータス・英語翻訳・タグ・外部リンクを段階的に追加できるようにします。一般公開には、日本語店舗名・日本語住所・都道府県・市区町村・緯度・経度・営業ステータスを必須とします。保存条件、型、API、セキュリティの確定内容は[管理店舗の下書き・公開設計](admin-pub-lifecycle.md)を参照してください。
