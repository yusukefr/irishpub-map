...
Quizは quiz_questions、quiz_question_translations、quiz_choices、quiz_choice_translations の4テーブルで管理します。QuestionはCategory、Special Date、Correct Choice、Source URL、Related Content、任意の画像参照、公開状態を持ち、各QuestionとChoiceはja/en翻訳を持ちます。問題画像は `media_assets` を参照し、日英の代替テキストと任意のキャプションをQuestion翻訳に保持します。画像未設定なら両言語の画像説明も空にします。別画像への変更時は説明を再入力します。
...
Published Questionは4 Choice、正解、日英のQuestion・Explanation・Source Label、HTTPSのSource URLを必須とします。画像を設定した場合は日英の代替テキストも必須です。Draftは入力途中の値を許容しますが、Public取得対象はPublishedだけです。AdminのPublish処理はRepositoryとDB内の条件で再検証します。画像は問題の理解を助けるものに限り、正解を示す画像や説明を避けます。
...
Server ComponentからClient Componentへ渡す情報は、Question ID、Category、問題文、任意の問題画像（URL・寸法・表示言語のaltとcaption）、Choiceだけです。Correct Choice、Explanation、Source、Related Contentは回答前のprops・HTML・serialized dataへ含めません。問題画像は問題文とChoiceの間に元画像の縦横比で表示し、captionがある場合だけ表示します。画像がない場合は余白を作りません。
...
E2EはProduction DBを使わず、E2E_TEST_MODEのfixtureでPublic・Admin・採点と画像表示を検証します。Draft QuestionはPublic fixtureからも除外します。
