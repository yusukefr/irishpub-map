...
既存の001適用済みDBは db/migrations/002_normalize_pub_metadata_up.sql、続けて db/migrations/003_municipality_codes_up.sql を明示的に実行します。003はリポジトリルートから市区町村コードCSVを psql のクライアント側コピーで取り込みます。適用後は各 migration の verify SQL で確認します。戻す場合は003のdown.sqlを先に実行します。
