...
管理者がMedia Assetを登録すると、画像バイナリは保存・配信のためVercel Blobへ送信され、公開URLで保存されます。管理APIは画像本体や資格情報をログへ出力しません。この管理者向け処理の送信先・目的は運用情報として記録し、一般利用者による投稿や画像表示機能を追加する場合は公開ページへの記載要否を改めて確認します。
