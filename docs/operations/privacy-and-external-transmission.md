...
管理者がMedia Assetを登録すると、画像バイナリは保存・配信のためVercel Blobへ送信され、公開URLで保存されます。公開記事の本文画像は同一OriginのMedia参照Routeを経てVercel Blobへredirectされ、利用者のブラウザからVercelへ画像リクエストが送られます。管理APIは画像本体や資格情報をログへ出力しません。公開用プライバシーページには既にVercelのコンテンツ配信を記載しており、送信先と目的は変わりません。
...
- 現在地の生の座標はアプリのサーバーやDBへ送信・保存しません。ただし、現在地周辺の地図を表示すると、周辺タイルの識別子が OpenFreeMap へ送信されます。

## 現在利用している送信・保存

### 利用者による共有

店舗詳細・公開Guideの共有操作では、公開タイトル・テキスト・正規URLをブラウザのWeb Share APIへ渡します。共有先は利用者がOS / ブラウザ上で選択します。アプリは共有先アプリ・アカウント・共有履歴を取得・保存せず、Instagram APIやOAuthを使用しません。非対応時のコピーは利用者の明示操作で端末のClipboardへ公開URLを書き込みます。共有操作用の独自Analyticsイベントやサーバー通信は追加しません。共有URLの閲覧には通常のページ配信・既存Analyticsが適用されます。
...
### OpenFreeMap ベクタースタイル

| 項目 | 整理結果 |
| --- | --- |
| 発生条件 | MapLibreが地図を表示・移動・拡大縮小すると発生する |
| 送信元 | 利用者のブラウザ。アプリサーバーを経由せず直接送信する |
| 送信先 | OpenFreeMap が運用する `tiles.openfreemap.org` と、配信に利用される場合がある Cloudflare CDN |
| 主な情報 | IPアドレス、ブラウザ・端末種別、OS、参照元、日時、要求したスタイル・フォント・スプライト・ベクタータイルのURL。タイルURLの `z/x/y` から表示地域と縮尺を推測できる |
| 目的 | ベクタースタイルと地図データの配信、サービスの運用・セキュリティ |
| 保持 | OpenFreeMap は通常のサーバーログに IPアドレスを保存しないと説明する。セキュリティ事象・不正利用の調査時には IP ログを最大30日保存し、解決後または30日後の早い方で削除するとしている。匿名化ログは改善目的で保持し得る |
| 削除 | OpenFreeMap のプライバシーポリシーに従う。Cloudflare が処理する情報・保存地域・個別削除手順は、このリポジトリだけでは確認できない |
...
地図タイル提供元の利用条件との照合、変更時の確認、商用プロバイダーまたはセルフホストへ切り替える基準は、[地図プロバイダー利用方針](map-provider.md)を参照してください。
...
- 現在地へ地図を移動した後のOpenFreeMapへのタイル要求については、前項のとおり間接的な地域情報の外部送信が発生します。
...
リポジトリだけでは次の項目を確定できません。現行の公開表示と計測を運用するため、プロジェクト所有者はVercel Dashboardと契約画面で定期的に確認してください。設定や契約を変更するときは、この文書と公開ページの記載も同じPull Requestで更新します。
...
電気通信事業法の外部送信規律について、このサービスがどの類型・規模で義務対象になるかは、この文書では最終判断しません。一方、OpenFreeMapの地図タイルやホスティングへの送信実態は、公開中の `/privacy` で利用者に確認できるようにしています。
...
- Web Analytics、Speed Insights、その他の計測・広告サービスを追加・変更する
...
## 公開Privacy Policyで確認・維持する項目

公開ページ `/privacy` の記載と、この文書の実態整理を照合するときの項目です。ここに挙げる項目がすべて公開ページへ掲載されていることを意味せず、公開ページへ反映する内容と内部運用上の確認事項を分けて管理します。
...
- OpenFreeMapへのタイル要求から表示地域を推測できること
...
公開用のプライバシー・外部送信ページは `/privacy` です。日本語を正本とし、画面の英語表示は日本語版を利用しやすくする補助として扱います。英語で新しい公開方針を提供する場合は、日本語正本と同じPull Requestで内容を照合して更新します。

ページ本文の最終更新日と変更履歴は、情報の取扱い、外部送信先、保持・削除方針、Cookie、現在地、計測機能のいずれかが変わるPull Requestで更新します。公開本文とこの調査文書が食い違う場合は、実装と一次資料を確認したうえで両方を同じPull Requestで修正します。

### 個人情報・外部送信の変更時チェックリスト

- [ ] 実装、ブラウザのNetwork、設定を確認し、実際に送信・保存する情報を特定した。
- [ ] 送信先、目的、送信経路、保持期間または期間を決める基準、削除・拒否方法を確認した。
- [ ] 現在地、Cookie、URLやクエリに個人情報・秘密情報が含まれないことを確認した。
- [ ] Web Analytics、Speed Insights、広告、その他の計測・外部サービスの追加または再開有無を確認した。
- [ ] 必要な通知、同意、対象地域、国外処理、契約・サブプロセッサを再確認した。
- [ ] `/privacy` の本文、最終更新日、変更履歴と、この文書、関連する仕様・テストを同じPull Requestで更新した。
