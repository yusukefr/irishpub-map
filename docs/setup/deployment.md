...
## Irish Calendar Data Migrationの履歴

Irish CalendarはRepository同梱JSONからNeon PostgreSQLへ一度限りのData Migrationを実施済みです。現在のCalendar EventとTranslationのSource of TruthはNeon PostgreSQLであり、通常のPreview / Production DeploymentではData Migrationを実行しません。

Data MigrationのRead-only確認、25 Events / 50 Translationsの完全移行済み確認、旧JSON・Migration Script・Migration専用Test・npm scriptのCleanupは完了しています。将来のSchema変更は通常の[Neon migration Runbook](../runbooks/neon-migrations.md)を利用します。
