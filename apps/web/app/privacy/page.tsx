...
const policySections = [
  {
    ...
    paragraphs: [
      ...
      "地図の表示時には、ブラウザからOpenFreeMapのサーバーおよび配信基盤へ直接通信が行われます。ブラウザ・端末情報、アクセス日時、参照元、地図表示に必要なリクエスト情報などが送信される場合があります。OpenFreeMapは通常のサーバーログにIPアドレスを保存しないと説明していますが、セキュリティ上の事案では最大30日間一時保存する場合があります。",
    ],
    links: [
      {
  ...
  },
  {
    ...
    paragraphs: [
      "本サービスでは、サービスの提供および改善に必要な範囲でOpenFreeMapとVercelを利用しています。OpenFreeMapは地図表示のために利用し、地図表示時には利用者のブラウザから直接通信します。Vercelはホスティング、コンテンツ配信、セキュリティ、障害調査、利用状況および性能の把握に利用します。",
      "本サービスの運営者が、OpenFreeMap側で取得された情報を独自に取得または保存することはありません。",
    ],
  ...
  },
  {
    title: "9. お問い合わせ",
    paragraphs: [
      "このプライバシーポリシーに関するお問い合わせは、Irish Pub Map の公開リポジトリの Issue からお願いします。個別のログやOpenFreeMap側の記録については、提供者の方針により対応できない場合があります。",
    ],
  },
];
